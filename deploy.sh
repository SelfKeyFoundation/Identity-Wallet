...
then
    ...
    gsutil cp /tmp/linux/dist/*.AppImage gs://selfkey-builds/$CIRCLE_BRANCH/$VERSION/$TIMESTAMP/
else
    VERSION=$(cat /tmp/mac/package.json | jq .version)
    gsutil cp /tmp/mac/dist/*.zip gs://selfkey-builds/$CIRCLE_BRANCH/$VERSION/$TIMESTAMP/
