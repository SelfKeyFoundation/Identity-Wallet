...
then
    ...
    curl -i -X POST -H "Content-Type: application/json" -d "{\"text\": \"Mac build has been deployed for \n$COMMIT_MESSAGE \n[(see artifacts)](https://console.cloud.google.com/storage/browser/sk-builds/$CIRCLE_BRANCH/$VERSION/$TIMESTAMP/?project=selfkey2)\", \"channel\": \"poi-builds\" }" $MATTERMOST_TOKEN
fi
