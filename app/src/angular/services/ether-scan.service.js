...
function EtherScanService(
	...
	const SERVER_CONFIG = {
		etehrscan: {
			1: {
				...
				key: "4C1HD9C8VKIAEPWFK9DIS6ZUAQTBE7PMUD"
			}, // chainId : { "api url", key )
			3: {
	...
	class EtherScanService {
		...
		sendRawTransaction(trxSignedHex) {
			...
			const apiUrl =
				SERVER_URL +
				"?module=proxy&action=eth_sendRawTransaction&tag=latest&apikey=" +
				API_KEY;
			let primise = $http.post(
				apiUrl,
				$httpParamSerializerJQLike({ hex: trxSignedHex }),
				REQUEST_CONFIG
			);
		...
		getCurrentBlock() {
			const apiUrl =
				SERVER_URL + "?module=proxy&action=eth_blockNumber&tag=latest&apikey=" + API_KEY;
			...
			$http
				...
				.then(response => {
					if (response.data.error || !response.data || !response.data.result) {
						defer.reject({
							message: $rootScope.getTranslation(
								ERROR_CODES[response.data.error.code]
							),
				...
				.catch(error => {
					defer.reject({
						message: $rootScope.getTranslation("http_connection_error")
					});
				});
		...
		getEthCall(data) {
			...
			const apiUrl =
				SERVER_URL +
				"?module=proxy&action=eth_call&to=" +
				data.to +
				"&data=" +
				data.data +
				"&tag=latest&apikey=" +
