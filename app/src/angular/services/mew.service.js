...
function MEWService($rootScope, $window, $q, $timeout, $log, $http, $httpParamSerializerJQLike, EVENTS, ElectronService, CommonService) {
    'ngInject';
        ...
        static handlePromise(defer, promise) {
            promise.then((response) => {
                if (response.error || response.errorMessage || !response.result) {
                    defer.reject($rootScope.buildErrorObject("mew_api_error", response.data.error));
                } else {
