...
function WalletService(
	...
	class WalletService {
		...
		importUsingKeystoreFileDialog() {
			...
			promise.then(resp => {
				this.importUsingKeystoreFilePath(resp.path)
					.then(wallet => {
						defer.resolve(wallet);
					})
					.catch(error => {
		...
		unlockKeystoreObject(password) {
			...
			let promise = ElectronService.unlockEtherKeystoreObject(
				wallet.keystoreObject,
				password
			);
			promise
				.then(privateKey => {
		...
		loadBalance() {
			...
			promise
				.then(balanceWei => {
					...
					wallet.balanceEth = Number(
						CommonService.numbersAfterComma(wallet.balanceEth, 8)
					);

					if (wallet.balanceWei !== balanceWei && !isFirstLoad) {
						ElectronService.showNotification(
							"Identity Wallet",
							"ETH Balance Changed " + wallet.balanceEth,
							{}
						);
					}
