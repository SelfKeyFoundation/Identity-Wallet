			...
			txInfoPromise
				.then(txInfo => {
					if (txInfo.blockNumber !== null) {
						$scope.backgroundProcessStatuses.checkingTransaction = Number(
							txInfo.status
						);
	...
	$scope.$watch(
		...
		(newVal, oldVal) => {
			...
			if (newVal.sendAmount && isNumeric(newVal.sendAmount)) {
				...
				// send amount in USD
				$scope.infoData.sendAmountInUSD =
					Number($scope.formData.sendAmount) * Number($scope.infoData.usdPerUnit);
