...
function MemberSetupChooseController(
  $rootScope,
  $scope,
  $log,
  $state,
  ...
  $scope.importIdentity = event => {
    ...
    promise.then(file => {
      ...
      ElectronService.importKYCIdentity(file)
        .then(resp => {
          publicAddress = Web3Service.constructor.web3.utils.toChecksumAddress(
            publicAddress
          )
          ...
          let sessionPromise = SelfkeyService.retrieveKycSessionToken(
            ...
            idAttributes['email'].items[idAttributes['email'].defaultItemId]
              .values[0], // TODO check email
