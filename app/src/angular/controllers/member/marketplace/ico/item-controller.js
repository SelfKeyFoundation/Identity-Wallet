function MemberMarketplaceIcoItemController(
	$rootScope,
	$scope,
	$log,
	$q,
	...
	$scope.gotToTokenSale = $event => {
		$state.go("member.marketplace.ico-accept-terms", {
			selected: $scope.ico
		});
	};
	...
	function normaliseIcoData() {
		if (typeof $scope.ico.token.totalOnSale === "number") {
			$scope.ico.tokenSalePercent = (
				$scope.ico.token.totalOnSale /
				$scope.ico.token.total *
				100
			).toFixed(2);
		}
		if ($scope.ico.cap.total && $scope.ico.cap.raised) {
			$scope.ico.cap.capPercent = (
				$scope.ico.cap.raised /
				$scope.ico.cap.total *
				100
			).toFixed(2);
		}
