...
function appStates(
	...
	function checkWallet(
		...
		if (!$rootScope.wallet || !$rootScope.wallet.getPublicKeyHex()) {
			...
			$rootScope.primaryToken = TokenService.getBySymbol(
				$rootScope.PRIMARY_TOKEN.toUpperCase()
			);
			 ...
			 */
			$q
				.all([loadPricesPromise, ethBalancePromise, keyBalancePromise])
				.then(() => {
					...
					$rootScope.balanceWatcherPromise = $interval(() => {
						if ($rootScope.wallet && $rootScope.wallet.getPublicKeyHex()) {
							SelfkeyService.getPrices(["ETH", "KEY"]).then(resp => {
								if ($rootScope.PRICES["ETH"]) {
									$rootScope.wallet.usdPerUnit =
										$rootScope.PRICES["ETH"].priceUsd;
								}

								if ($rootScope.PRICES["KEY"]) {
									$rootScope.primaryToken.usdPerUnit =
										$rootScope.PRICES["KEY"].priceUsd;
								}
							});
	...
	function checkKyc(
		...
		if (
			!walletStore.data ||
			!walletStore.data.idAttributes ||
			Object.keys(walletStore.data.idAttributes).length <= 0
		) {
