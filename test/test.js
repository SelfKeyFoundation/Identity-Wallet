...
app.start().then(() => {
	...
	c.waitUntilWindowLoaded()
		.then(() => console.log(chalk.green('Start Create New Wallet From Password Flow')))
...
// Click Create New Wallet
		.then(() => delay(1000).then(() => c.waitForVisible('#createWallet', 10000)))
		.then(() => c.click('#createWallet'))

// 2x Password
		.then(() => delay(1000).then(() => c.waitForVisible('#pw1', 10000)))
...
// click download
		...
		.then(() => delay(1000).then(() => c.waitForVisible('#dl', 10000)))
		.then(() => c.click('#dl'))
		//click next
		.then(() => c.click('#next'))
...
// assert vs ?
		// click next
		.then(() => delay(1000).then(() => c.waitForVisible('#next2', 10000)))
		.then(() => c.click('#next2'))
