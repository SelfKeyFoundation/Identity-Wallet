...
describe("Creates a New Wallet with Basic ID Details and a Password", () => {
    ...
    it("Step 2: Fills Out the Basic ID Form", () => {
        return delay(1000)
            ...
            .then(() =>
                tools.app.client.setValue("#firstName", data[0].firstName)
            )
            .then(() =>
                tools.app.client.setValue("#lastName", data[0].lastName)
            )
            .then(() =>
                tools.app.client.setValue("#middleName", data[0].middleName)
            )
