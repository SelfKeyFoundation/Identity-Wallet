...
import testRequirements from './__fixtures__/test-requirements';
		...
		describe('submitKycRequirements', () => {
			it('should submit kyc requirements', async () => {
				...
				const updateInfo = {
					firstName: 'test',
					lastName: 'Last',
					phoneNumber: '+380934457350',
					dateOfBirth: '1991-07-15T00:00:00.000Z',
					address: {
						street: 'test app 1',
						subStreet: null,
						town: 'Idan',
				...
				sinon.stub(api, 'uploadFile').resolves();
				const res = await service.submitKycRequirements(testRequirements, auth);
				expect(res).toEqual(expected);
				expect(service.getApi.getCall(0).args[0]).toEqual(auth);
				expect(l.getCall(0).args[0]).toEqual(updateInfo);
				expect(api.uploadFile.callCount).toBe(2);
			});
		});
	});
});
