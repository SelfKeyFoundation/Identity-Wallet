...
module.exports = function(app) {
    ...
    */
	...
	// NEED TEST
	controller.prototype.getTransactionsHistoryByWalletId = function(
		event,
		actionId,
		actionName,
		args
	) {
		electron.app.sqlLiteService.TransactionHistory.findByWalletId(args.walletId)
			.then(data => {
				app.win.webContents.send(RPC_METHOD, actionId, actionName, null, data);
			})
			.catch(error => {
	...
	// NEED TEST
	controller.prototype.getTransactionsHistoryByWalletIdAndTokenId = function(
		event,
		actionId,
		actionName,
		args
	) {
		electron.app.sqlLiteService.TransactionHistory.findByWalletIdAndTokenId(
			args.walletId,
			args.tokenId
		)
			.then(data => {
	...
	controller.prototype.insertTransactionHistory = function(event, actionId, actionName, args) {
		electron.app.sqlLiteService
			.transactionsHistory_insert(args)
			.then(data => {
				app.win.webContents.send(RPC_METHOD, actionId, actionName, null, data);
			})
			.catch(error => {
