...
function Web3Service($rootScope, $window, $log, EVENTS, SqlLiteService, RPCService) {
	...
	class Web3Service {
		...
		syncTokensTransactionHistory(tokenSymbol) {
			...
			$rootScope.transactionHistorySyncStatuses =
				$rootScope.transactionHistorySyncStatuses || {};

			let getActivity = (contractAddress, fromBlock, toBlock, filter) => {
				return Web3Service.getContractPastEvents(contractAddress, [
					'Transfer',
					{
						filter: filter,
						fromBlock: fromBlock,
			...
			Object.keys(tokens).forEach(key => {
				...
				let processAllActivities = (fromBlock, toBlock) => {
					// process from
					getActivity(token.contractAddress, fromBlock, toBlock, {
						from: walletAddress
					}).then(logsFrom => {
						//process to
						getActivity(token.contractAddress, fromBlock, toBlock, {
							...
							(function next(err) {
								if (/*|| err*/ !transactions.length) {
									...
									SqlLiteService.getWalletSettingsByWalletId(wallet.id)
										.then(settings => {
											...
											SqlLiteService.saveWalletSettings(setting).catch(
												err => {
								...
								Web3Service.getBlock(blockNumber, true).then(blockData => {
									let transactionFromBlok = blockData.transactions.find(
										blockTransaction => {
									...
									let newTransaction = {
										...
										value: Number(
											new BigNumber(transaction.returnValues.value)
												.div(valueDivider)
												.toString()
										),
										txId: txId,
